'''
Structural functions needed for the NN:
...
linear_backward(dA, cache):
	dA, dW, db

activation_forward(A_prev, W, b, activation='relu'):
	A, cache

activation_backward(dA, cache, activation='relu'):
	dA_prev, dW, db

L_forward(X, parameters):
	AL, caches

L_backward(AL, Y, caches):
	grads

NOT:
L_layer_model(X, Y, layer_dims, learning_rate=0.01, num_iterations= , print_cost= )
	parameters
'''
